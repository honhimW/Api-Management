...
export const openAPI3toMd = (jsonObj) => {
    var map = openAPI3trans(jsonObj)
    var controllerMap = map.tableMap
    var sb = ''
    sb += '# ' + map.info.title + '\n\n'
    sb += '> Version  : ' + map.info.version + '\n\n'
  ...
  export const openAPI3trans = (jsonObj) => {
    // var jsonObj = JSON.parse(jsontxt)
    var attrs = parseSchemas(jsonObj)
    ...
    if (paths !== undefined) {
      Object.keys(paths).forEach(pathKey => {
        ...
        var description = defaultIfBlank(content.description, content.summary)
        ...
        var requestBody = content.requestBody
        ...
        if (requestBody !== undefined) {
          var ref = requestBody.content['application/json'].schema.$ref
          parameters.push({
            name: ref.substring(COMPONENTS_SCHEMAS_PREFIX.length),
            in: 'body',
            required: true,
            schema: {
              type: 'object',
        ...
        var table = {
          ...
          responseForm,
          requestType,
          requestList: processRequestList(parameters, attrs),
          responseList: processResponseList(responses)
        }
        table.modelAttr = {
          className: '',
          name: '',
        ...
        if (too !== undefined && too.content !== undefined) {
    ...
    resultMap.basePath = jsonObj.servers[0].url
    return resultMap
  }
  
  const parseSchemas = (origin) => {
    var schemas = origin.components.schemas
    var attrs = {}
    if (schemas !== undefined) {
      Object.keys(schemas).forEach(modelName => {
        fillModel(schemas, attrs, modelName)
      })
    }
    return attrs
  }
  const COMPONENTS_SCHEMAS_PREFIX = '#/components/schemas/'
  const fillModel = (schemas, attrs, modeName) => {
    var attr = attrs[COMPONENTS_SCHEMAS_PREFIX + modeName]
    if (attr === undefined) {
      attr = {}
      attrs[COMPONENTS_SCHEMAS_PREFIX + modeName] = attr
    } else if (attr.isCompleted) {
      return attr
    }
    var properties = schemas[modeName].properties
    var required = schemas[modeName].required
    ...
    Object.keys(properties).forEach(key => {
      ...
      if (ref !== undefined || (items !== undefined && (ref = items.$ref) !== undefined)) {
        var clzName = ref.substring(COMPONENTS_SCHEMAS_PREFIX.length)
        var refModel = fillModel(schemas, attrs, clzName)
    ...
    var title = schemas[modeName].title
    var description = schemas[modeName].description
  ...
  const processRequestList = (parameters, attrs) => {
    ...
    if (parameters !== undefined && parameters.length > 0) {
      parameters.forEach(param => {
        ...
        request.type = defaultIfBlank(param.schema.type, 'object')
        ...
        if (_in !== undefined && _in === 'body') {
          ...
          if (ref !== undefined) {
            request.type = request.type + ':' + ref.replaceAll(COMPONENTS_SCHEMAS_PREFIX, '')
  ...
  const processResponseModelAttrs = (too, attrs) => {
    var firstKey = Object.keys(too.content).find(key => true)
    var schema = too.content[firstKey].schema
