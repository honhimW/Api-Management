...
!(function () {
  ...
  var grammar = (function () {
    ...
    var parser = {
        ...
        parse: function (input) {
          function lex () {
            var token
            return token = self.lexer.lex() || EOF, typeof token !== 'number' && (token = self.symbols_[token] || token), token
          }
          var self = this,
            stack = [0],
            vstack = [null],
          ...
          this.parseError = typeof this.yy.parseError === 'function' ? this.yy.parseError : Object.getPrototypeOf(this).parseError
          for (var symbol, preErrorSymbol, state, action, r, p, len, newState, expected, yyval = {}; ;) {
            if (state = stack[stack.length - 1], this.defaultActions[state] ? action = this.defaultActions[state] : ((symbol === null || typeof symbol === 'undefined') && (symbol = lex()), action = table[state] && table[state][symbol]), typeof action === 'undefined' || !action.length || !action[0]) {
              var errStr = ''
              expected = []
            ...
            switch (action[0]) {
              ...
              case 2:
                ...
                }, ranges && (yyval._$.range = [lstack[lstack.length - (len || 1)].range[0], lstack[lstack.length - 1].range[1]]), r = this.performAction.call(yyval, yytext, yyleng, yylineno, this.yy, action[1], vstack, lstack), typeof r !== 'undefined') return r
  ...
  typeof require !== 'undefined' && typeof exports !== 'undefined' && (exports.parser = grammar, exports.Parser = grammar.Parser, exports.parse = function () {
    return grammar.parse.apply(grammar, arguments)
  }, exports.main = function (args) {
    args[1] || (console.log('Usage: ' + args[0] + ' FILE'), process.exit(1))
    var source = require('fs').readFileSync(require('path').normalize(args[1]), 'utf8')
    return exports.parser.parse(source)
  }, typeof module !== 'undefined' && require.main === module && exports.main(process.argv.slice(1))), ParseError.prototype = new Error(), Diagram.ParseError = ParseError, grammar.parseError = function (message, hash) {
    throw new ParseError(message, hash)
  }, Diagram.parse = function (input) {
    return grammar.yy = new Diagram(), grammar.parse(input)
  }, this.Diagram = Diagram
}.call(this)), typeof jQuery !== 'undefined' && (function ($) {
  ...
  _.extend(BaseTheme.prototype, {
    ...
    layout: function () {
      function actor_ensure_distance (a, b, d) {
        assert(b > a, 'a must be less than or equal to b'), a < 0 ? (b = actors[b], b.x = Math.max(d - b.width / 2, b.x)) : b >= actors.length ? (a = actors[a], a.padding_right = Math.max(d, a.padding_right)) : (a = actors[a], a.distances[b] = Math.max(d, a.distances[b] ? a.distances[b] : 0))
      }
      var diagram = this.diagram,
        paper = this._paper,
        font = this._font,
      ...
      _.each(actors, function (a) {
        ...
        var extra_width = 0
        if (s.type == 'Signal') s.width += 2 * (SIGNAL_MARGIN + SIGNAL_PADDING), s.height += 2 * (SIGNAL_MARGIN + SIGNAL_PADDING), s.isSelf() ? (a = s.actorA.index, b = a + 1, s.width += SELF_SIGNAL_WIDTH) : (a = Math.min(s.actorA.index, s.actorB.index), b = Math.max(s.actorA.index, s.actorB.index))
        else {
          if (s.type != 'Note') throw new Error('Unhandled signal type:' + s.type)
      ...
      return _.each(actors, function (a) {
        a.x = Math.max(actors_x, a.x), _.each(a.distances, function (distance, b) {
          typeof distance !== 'undefined' && (b = actors[b], distance = Math.max(distance, a.width / 2, b.width / 2), b.x = Math.max(b.x, a.x + a.width / 2 + distance - b.width / 2))
    ...
    draw_signals: function (offsetY) {
      ...
      _.each(this.diagram.signals, function (s) {
        s.type == 'Signal' ? s.isSelf() ? this.draw_self_signal(s, y) : this.draw_signal(s, y) : s.type == 'Note' && this.draw_note(s, y), y += s.height
      }, this)
    },
